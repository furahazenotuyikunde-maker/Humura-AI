...
import { 
  ...
  FileText, Activity, Clock, Shield, BarChart2, Video,
...
import BrailleGenerator from '../components/doctor/BrailleGenerator';
...
export default function DoctorDashboard() {
  ...
  const handleDownloadReport = (report: any, patientName: string) => {
    if (!report) return;
    
    const content = `
HUMURA AI - CLINICAL PROGRESS REPORT
Patient: ${patientName}
Date: ${new Date().toLocaleDateString()}
------------------------------------------
MOOD TREND: ${report.mood_trend?.toUpperCase() || 'N/A'}
AVERAGE MOOD SCORE: ${report.avg_mood_score || '0'}/10

CLINICAL AI SUMMARY:
${report.ai_summary || 'No summary generated.'}
...
Generated by Humura AI
    ...
    link.href = url;
    link.download = `Clinical_Report_${patientName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.txt`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    showToast(isRw ? 'Raporo yamanuwe' : 'Report downloaded successfully', 'success');
  };
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFB] flex">
      ...
      <aside className="w-64 bg-white border-r border-[#E8E1DB] flex flex-col p-6 fixed h-full z-50">
        ...
        <nav className="flex-1 space-y-1">
          {[
            ...
            { id: 'videos', icon: Video, label: isRw ? 'Amashusho' : 'Educational Videos' },
            { id: 'braille', icon: Settings, label: isRw ? 'Inyuguti za Braille' : 'Braille Generator' },
          ...
          <div className="flex-1 space-y-8">
            ...
            {activeTab === 'dashboard' && (
              <>
                ...
                {/* Quick Actions Card */}
                <div className="bg-white p-8 rounded-[2.5rem] border border-[#E8E1DB] space-y-6">
                  <h3 className="font-black text-[#4A2C1A] uppercase text-[10px] tracking-widest">{isRw ? 'Ibikorwa byihuse' : 'Quick Actions'}</h3>
                  <div className="flex gap-4">
                    <button 
                      onClick={() => setActiveTab('sessions')}
                      className="flex-1 py-4 bg-primary text-white font-black uppercase text-xs rounded-2xl shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                    >
                      <Activity size={16} />
                      {isRw ? 'Tangira Ikiganiro' : 'Start Session'}
                    </button>
                    <button 
                      onClick={() => setActiveTab('patients')}
                      className="flex-1 py-4 bg-white border border-[#E8E1DB] text-[#4A2C1A] font-black uppercase text-xs rounded-2xl hover:bg-neutral-50 transition-all flex items-center justify-center gap-2"
                    >
                      <Users size={16} />
                      {isRw ? 'Amateka' : 'View History'}
                    </button>
                  </div>
                </div>
            ...
            {activeTab === 'patients' && (
              <div className="bg-white rounded-[2.5rem] border border-[#E8E1DB] p-8 h-[calc(100vh-200px)]">
                <PatientManagement 
                  ...
                  onStartSession={async (patientId) => {
                    try {
                      ...
                      if (error) throw error;
                      showToast(isRw ? 'Ikiganiro cyateguwe' : 'Session created and scheduled for now', 'success');
                      fetchInitialData();
      ...
      <ClinicalReportModal 
        ...
        onDownload={(report: any) => handleDownloadReport(report, selectedPatient?.patient_info?.full_name || 'Patient')}
...
// --- Report Modal ---
const ClinicalReportModal = ({ isOpen, onClose, patient, onGenerate, onDownload, loading, report, isRw }: any) => {
