...
export default function SignLanguagePage() {
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            const fallbackRes = await fetch(
              `https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-preview:generateContent?key=${apiKey}`,
              {
                ...
                body: JSON.stringify({
                  contents: [{
                    parts: [
                      { text: `Analyze this sign language gesture and return JSON: {"detectedSign": "...", "explanation": "..."}` },
            ...
            if (!fallbackRes.ok) throw new Error("Vision fallback failed");
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      try {
        ...
        const fallbackRes = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`,
          {
            ...
            body: JSON.stringify({
              contents: [{
                parts: [{
                  text: `The user selected these signs: ${message}. Provide a compassionate, CBT-based response in ${isRw ? 'Kinyarwanda' : 'English'}.`
                }]
              }]
            })
          }
        ...
        if (!fallbackRes.ok) throw new Error("Chat fallback failed");
        const fbData = await fallbackRes.json();
        const fbReply = fbData.candidates?.[0]?.content?.parts?.[0]?.text;
        setAiResponse(fbReply || generateFallback());
      } catch (fe: any) {
        console.error('[FALLBACK] ✖ Error:', fe.message);
  ...
  useEffect(() => {
    if (autoDetectActive && cameraActive) {
      detectIntervalRef.current = setInterval(captureAndAnalyze, 15000);
    } else {
      if (detectIntervalRef.current) clearInterval(detectIntervalRef.current);
    }
      ...
      <div className="glass-card rounded-2xl p-4 overflow-hidden relative">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-colors ${cameraActive ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-primary-50 text-primary hover:bg-primary-100'
              }`}
          >
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
        {(Object.entries(CATEGORY_CONFIG) as [keyof typeof CATEGORY_CONFIG, any][]).map(([id, config]) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl whitespace-nowrap text-xs font-bold transition-all ${activeCategory === id
        ...
        {categoryFiltered.map(sign => {
          ...
          return (
            <motion.button
              ...
              className={`relative flex flex-col items-center justify-center p-4 rounded-3xl transition-all border-2 ${isSelected
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {selected.map(sign => (
                <div
                  key={sign.id}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium ${sign.isCrisis ? 'bg-red-100 text-red-800' : 'bg-primary-100 text-primary-800'
                    }`}
                >
